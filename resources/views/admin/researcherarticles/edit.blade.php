...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('researcherarticles.update', $researcherarticle->id) }}" id="storeform" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PATCH")
                <div class="form-group mb-3">
                    <label for="title">Yangi maqola nomi</label>
                    <input type="text" class="form-control" id="title" name="article_topic" value="{{ $researcherarticle->article_topic }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="title">Yozuvchilar</label>
                    <input type="text" class="form-control" id="title" name="authors" value="{{ $researcherarticle->authors }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="title">Maqola turi</label>
                    <input type="text" class="form-control" id="title" name="article_type" value="{{ $researcherarticle->article_type }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="title">Himoya sanasi</label>
                    <input type="text" class="form-control" id="title" name="protection_year" value="{{ $researcherarticle->protection_year }}" required>
                </div>
                <div class="row mb-3">
                                ...
                                <div class="tab-pane fade" id="horizontal-url">
                                    <div class="input-group">
                                        ...
                                        <input
                                            ...
                                            value="{{ $researcherarticle->file_url }}"
                                            id="inputGroupFile01"
                                        />
                                    </div>
                                    @error('file_url')
