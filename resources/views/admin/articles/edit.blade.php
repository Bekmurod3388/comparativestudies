...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('articles.update', $article->id) }}" id="storeform" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PATCH")
                <div class="form-group mb-3">
                    <label for="title">Maqola nomi</label>
                    <input type="text" class="form-control" id="title" name="name" value="{{ $article->name }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="title">Chop etilgan jurnal nomi</label>
                    <input type="text" class="form-control" id="title" name="journal_name" value="{{ $article->journal_name }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="title">Yozuvchilar</label>
                    <input type="text" class="form-control" id="title" name="authors" value="{{ $article->authors }}" required>
                </div>
                <div class="form-group mb-3">
                    <label for="type">Maqola tili</label>
                    <select class="form-control" id="type" name="locale_id" required onchange="selectType(this)">
                        @forelse($locales as $locale)
                            <option value="{{ $locale->id }}" {{ $locale->id == $article->locale_id ? 'selected' : '' }}>
                                {{ $locale->name }}
                            </option>
                        @empty
                ...
                ><i class="bx bx-hourglass"></i
                    ></span>
                            <input
                                ...
                                value="{{ Carbon::parse($article->published_date)->format('Y-m-d') }}"
                            />
                        </div>
